...
class NewsfeedView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            newsfeed_queryset = NewsfeedIndex.objects.filter(
                author_id__in=connections_list
            ).limit(int(page_size))

            candidate_feed_ids = [feed.post_id for feed in newsfeed_queryset]
            ...
            queryset = (
                ...
                .filter(post_id__in=candidate_feed_ids)
                .order_by(
                    "-score__ranking_score",
                )
            )
